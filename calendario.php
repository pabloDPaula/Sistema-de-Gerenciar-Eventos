<?php
    ...
    if((!isset($_SESSION['email']) == true) and (!isset($_SESSION['senha']) == true)){
        ...
        $eventos = "SELECT e.* FROM eventos as e inner join participantes as c on e.idEvento = c.idEvento 
        WHERE c.idUsuario = $idUsuario";
        $resultEventos = $conexao->query($eventos);

        // Apartir do método GET, pegamos a variável idEvento passada por parâmetro e verifica se não está vazia
        if(!empty($_GET['idEvento'])){ 
           $idEvento = $_GET['idEvento'];
           $desinscrever = "DELETE FROM `participantes` WHERE idEvento = $idEvento and idUsuario = $idUsuario";
           $result = $conexao->query($desinscrever);
    ...
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
      ...
      <div class="container conteudo">
        <div class="row mt-5 mb-4">
            <h2><strong>Eventos Inscrito</strong></h2>
        </div>
        <div class="row">
            <?php         
                while($eventosDB = mysqli_fetch_assoc($resultEventos)){
                    ...
                    echo "</div>";
                    echo "<div class='p-3'>";
                    echo "<a type='button' class='btn btn-danger ms-3' href='calendario.php?idEvento=$eventosDB[idEvento]'>Desinscrever-se</a>";
                    echo "</div>";
                    echo "</div>";
                }         
            ?>
