...
if ((!isset($_SESSION['email']) == true) and (!isset($_SESSION['senha']) == true)) {
    ...
    if (!empty($_GET['idEvento'])) {
        ...
        $inscrever = "INSERT INTO participantes(idUsuario,idEvento) VALUES ($idUsuario,$idEvento)";
        $result = $conexao->query($inscrever);
    }
}
...
<body>
    ...
    <div class="container conteudo">
        <div class="row mt-5 mb-4">
            <h2><strong>Eventos Disponíveis</strong></h2>
        </div>
        <div class="row">
            <?php
            ...
            while ($eventosDB = mysqli_fetch_assoc($resultEventos)) {
                ...
                echo "<div class='card m-4 mt-5' style='width: 15rem;'>";
                echo "<div class='p-3'>";
                echo "<h4 class='card-title'><strong>" . $eventosDB['nome'] . "</strong></h4>";
                echo "</div>";
                ...
                $idEvento = $eventosDB['idEvento'];
                $sql = "SELECT * FROM participantes WHERE idUsuario = $idUsuario and idEvento= $idEvento";
                $result = $conexao->query($sql);

                // Muda o botão entre inscrito e inscrever-se

                if (mysqli_num_rows($result) < 1) {
                    echo "<a type='button' class='btn btn-danger ms-4' href='home.php?idEvento=$eventosDB[idEvento]'>Inscrever-se</a>";
                } else {
                    echo "<a type='button' class='btn btn-danger disabled  ms-5'>Inscrito</a>";
                }
                echo "</div>";
                echo "</div>";
            }
